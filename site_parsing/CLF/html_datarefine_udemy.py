...
df = pd.DataFrame(columns=['exam_category',"exam_type","quiz_id",'en_quiz_title','most_vote',"correct","en_A","en_B","en_C","en_D","en_E","en_F","ko_quiz_title","ko_A","ko_B","ko_C","ko_D","ko_E","subject","choice_count"])


choice_alphabet_list = ['A','B','C','D','E','F']
udemy_dir = "./ud"


for i in tqdm(range(1,7)):
    with open(os.path.join(udemy_dir,"6 Practice Exams _ AWS Certified Cloud Practitioner CLF-C01 _ Udemy_"+str(i)+".html"), "r", encoding="utf-8") as file:
        # 파일 내용 읽기
        html_data = file.read()
...
# df.to_csv('./UDEMY_CLF_quiz_data.csv',index=False)
...
for i in tqdm(range(1,7)):
    with open(os.path.join(udemy_dir,"6 연습 시험 _ AWS 공인 클라우드 전문가 CLF-C01 _ 유데미_"+str(i)+".html"), "r", encoding="utf-8") as file:
        # 파일 내용 읽기
        html_data = file.read()
