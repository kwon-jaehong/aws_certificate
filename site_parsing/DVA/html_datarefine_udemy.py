...
for i in tqdm(range(2,7)):
    with open(os.path.join(udemy_dir,"AWS Certified Developer Associate Practice Exams _ Udemy_e"+str(i)+".html"), "r", encoding="utf-8") as file:
        # 파일 내용 읽기
        html_data = file.read()
...
count = 0
for i in tqdm(range(2,7)):
    with open(os.path.join(udemy_dir,"AWS Certified Developer Associate Practice Exams _ Udemy_"+str(i)+".html"), "r", encoding="utf-8") as file:
        # 파일 내용 읽기
        html_data = file.read()
...
df['subject'] = 0
df.to_csv('./UDEMY_DVA_quiz_data.csv',index=False)
